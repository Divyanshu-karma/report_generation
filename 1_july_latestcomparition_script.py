...
def normalize_address(addr: str) -> str:
    if not addr:
        return ""
    addr = str(addr).upper()
    # Collapse all spacing variations, tabs, carriage returns, newlines into a single space
    addr = re.sub(r'[\r\n\s]+', ' ', addr)
    # Strip punctuation, commas, hyphens
    addr = re.sub(r'[.,\-]', '', addr)
    return addr.strip()
...
def business_name_tiebreak_score(rec2: NormalizedRecord, cand: NormalizedRecord) -> float: 
    ...
    # Address comparison
    addr2 = rec2.raw_payload.get("address")
    src_data = cand.raw_payload.get("source_specific_data")
    addr1 = None
    if src_data:
        if isinstance(src_data, dict):
            addr1 = src_data.get("full_address")
        elif isinstance(src_data, str):
            try:
                parsed_src = json.loads(src_data)
                addr1 = parsed_src.get("full_address")
            except Exception:
                pass
